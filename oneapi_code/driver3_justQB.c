...
int main(int argc, char **argv)
{
    ...
    if (!parse_dimension(argv[1], &m) || !parse_dimension(argv[2], &n)) {
        fprintf(stderr, "Error: rows and cols must be positive integers within the LP64 MKL range.\n");
        print_usage(argv[0]);
        return EXIT_FAILURE;
    }
    ...
    while (argi < argc) {
        if (strcmp(argv[argi], "--tolerance") == 0) {
            ...
            if (!parse_dimension(argv[argi + 1], &kstep)) {
                fprintf(stderr, "Error: kstep must be a positive integer within the LP64 MKL range.\n");
                print_usage(argv[0]);
                return EXIT_FAILURE;
            }
            kstep_supplied = 1;
