...
/* host2device and device2host data copy functions */
...
void matrix_matrix_mult(mat *A, mat *B, mat *C){
	...
	cublasDgemm(handle,
                ...
                &beta,
                C->ddev, C->nrows);
	matrix_elems_copy_from_device(C);
}
...
void matrix_vector_mult(mat *M, vec *x, vec *y){
	...
	cublasDgemv(handle, CUBLAS_OP_N,
                           ...
                           &beta,
                           y->ddev, 1);
	vector_elems_copy_from_device(y);
}
